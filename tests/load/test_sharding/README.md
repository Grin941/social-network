...
1. пользователь отправляет сообщение и user_id друга, кому хочет отправить сообщение (POST /dialog/{user_id}/send)
...
1. пользователь отправляет запрос с user_id друга, чат с которым хочет прочитать (GET /dialog/{user_id}/list)
2. ищем чат с двумя участниками request_user.id и user_id
3. если чат не найден, возвращаем 400
